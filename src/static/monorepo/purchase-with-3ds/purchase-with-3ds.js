...
function start3DSLifecycle(transactionToken) {
  ...
  lifecycle = new SpreedlyThreeDSLifecycle({
    ...
    callbacks: {
      onChallenge: (context) => {
        logEvent('Challenge required - showing modal', 'info');
        setPayButtonLoading(true, 'Complete verification...');
        showChallengeModal();
      },
      
      onSuccess: (transaction) => {
        logEvent('3DS authentication successful', 'success');
        hideChallengeModal();
        showResult('success', transaction);
      },
      ...
      onError: (error) => {
        ...
        logEvent(`3DS error: ${errorMsg}`, 'error');
        hideChallengeModal();
        showResult('error', {message: errorMsg});
      },
      
      onTimeout: () => {
        logEvent('3DS timeout', 'error');
        hideChallengeModal();
        showResult('error', { message: 'Authentication timed out. Please try again.' });
      },
    },
  });
