...
never included.** Mix in merchant-owned fields and `submitParams`, then call `submit()`.
Hosted keys in that `formData` are still stripped (hosted wins) and re-read inside the
number iframe. Validation, throttling, and tokenization are unchanged.
